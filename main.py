...
def caihongpi():
    if (Whether_caihongpi != False):
        try:
            ...
            params = urllib.parse.urlencode({'key': '63ab60928abec174ce5336740ea5bfbb'})
...
def lucky():
    if (Whether_lucky != False):
        #try:
            conn = http.client.HTTPSConnection('api.tianapi.com')  # 接口域名
            params = urllib.parse.urlencode({'key': '63ab60928abec174ce5336740ea5bfbb', 'astro': config["astro"]})
            headers = {'Content-type': 'application/x-www-form-urlencoded'}
...
def tip():
    if (Whether_tip != False):
        try:
            conn = http.client.HTTPSConnection('api.tianapi.com')  # 接口域名
            params = urllib.parse.urlencode({'key': '63ab60928abec174ce5336740ea5bfbb', 'city': city})
            headers = {'Content-type': 'application/x-www-form-urlencoded'}
...
if __name__ == "__main__":
    ...
    # 星座运势
    lucky_ = lucky()
    print(accessToken, city, weather, max_temperature, min_temperature, pipi, lizhi, pop, tips,
                     note_en, note_ch, health_tip, lucky_)
    # 公众号推送消息
    for user in users:
